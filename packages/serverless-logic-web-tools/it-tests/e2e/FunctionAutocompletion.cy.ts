...
describe("Serverless Logic Web Tools - Function Autocompletion test", () => {
  beforeEach(() => {
    cy.visit("/");
  });

  it("should invoke content assist and check autocompletion for functions", () => {
    // open prepared sw json file
    cy.get("#upload-field").attachFile("functionAutocompletion/functionAutocompletion.sw.json", {
    ...
    cy.getEditor().within(() => {
      cy.iframe("#kogito-iframe[src='./serverless-workflow-text-editor-envelope.html']").within(() => {
        // create path to row 10, column 30
        var path = "";
