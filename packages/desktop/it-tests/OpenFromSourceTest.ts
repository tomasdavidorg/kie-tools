...
test("BPMN from source", async () => {
  ...
  await urlInput.setValue(
    "https://raw.githubusercontent.com/kiegroup/kogito-tooling/master/packages/chrome-extension-pack-kogito-kie-editors/it-tests/samples/test.bpmn"
  );
...
test("DMN from source", async () => {
  ...
  const urlInput = await client.$("#url-text-input");
  await urlInput.setValue(
    "https://raw.githubusercontent.com/kiegroup/kogito-tooling/master/packages/chrome-extension-pack-kogito-kie-editors/it-tests/samples/test.dmn"
  );
  const openFromSourceButton = await client.$("[data-ouia-component-id='open-from-source-button']");
  await openFromSourceButton.click();
